...
<div class="totals-summary w-full lg:pl-[4rem] lg:pr-[6rem] py-8 lg:py-0">
    <table class="w-full text-right table-fixed hidden lg:table">
        ...
        @if($subtotal)
            <tr>
                <td colspan="4"></td>
                <td class="text-sm p-2 font-semibold text-gray-950 dark:text-white">{{ translate('Subtotal') }}:</td>
        ...
        @if($taxTotal)
            <tr>
                <td colspan="4"></td>
                <td class="text-sm p-2">{{ translate('Tax') }}:</td>
                <td class="text-sm p-2">{{ $taxTotal }}</td>
            </tr>
        @endif
        @if($isPerDocumentDiscount)
            <tr>
                <td colspan="3" class="text-sm p-2">{{ translate('Discount') }}:</td>
        ...
        <tr>
            <td colspan="4"></td>
            <td class="text-sm p-2 font-semibold text-gray-950 dark:text-white">{{ $amountDue ? translate('Total') : translate('Grand Total') }}:</td>
            <td class="text-sm p-2">{{ $grandTotal }}</td>
        </tr>
        @if($amountDue)
            <tr>
                <td colspan="4"></td>
                <td class="text-sm p-2 font-semibold text-gray-950 dark:text-white border-t-4 border-double">{{translate('Amount Due')}}
                    ({{ $currencyCode }}):
                </td>
        ...
        <div class="flex flex-col space-y-6">
            @if($subtotal)
                <div class="flex justify-between items-center">
                    <span class="text-sm font-semibold text-gray-950 dark:text-white">{{ translate('Subtotal') }}:</span>
                    <span class="text-sm">{{ $subtotal }}</span>
                </div>
            @endif
            @if($taxTotal)
                <div class="flex justify-between items-center">
                    <span class="text-sm">{{ translate('Tax') }}:</span>
                    <span class="text-sm">{{ $taxTotal }}</span>
                </div>
            @endif
            @if($isPerDocumentDiscount)
                <div class="flex flex-col space-y-2">
                    <span class="text-sm">{{ translate('Discount') }}:</span>
                    <div class="flex justify-between space-x-2">
                        @foreach($getChildComponentContainer()->getComponents() as $component)
            ...
            @endif
            <div class="flex justify-between items-center">
                <span class="text-sm font-semibold text-gray-950 dark:text-white">{{ $amountDue ? translate('Total') : translate('Grand Total') }}:</span>
                <span class="text-sm">{{ $grandTotal }}</span>
            </div>
            @if($amountDue)
                <div class="flex justify-between items-center">
                    <span
                        class="text-sm font-semibold text-gray-950 dark:text-white">{{ translate('Amount Due') }} ({{ $currencyCode }}):</span>
